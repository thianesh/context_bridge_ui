...
const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJyb2xlIjoiYW5vbiIsImlzcyI6InN1cGFiYXNlIiwiaWF0IjoxNzUwNzg5ODAwLCJleHAiOjE5MDg1NTYyMDB9.P3DyzAgVDhoS5upMxHAE9_kVgSqZbKCkR0Bakc4EGC0";
...
export const root_store = defineStore("root", () => {
  ...
  async function add_member(user_email, user_role) {
    ...
    if (lookupError || !userLookup) {
      ...
      return {
        status: false,
        message:
          "User not found. Please ensure user signed up in our platform.",
      };
    }
  ...
  async function update_room(room_id, room_name, room_members) {
    ...
    const userIds = room_members.map((m) => m.user_id).filter(Boolean); // remove null/undefined
  ...
  async function get_companies() {
    ...
    const { data: company, error: errorCompany } = await supabase
      .from("companies")
      .select("*")
      .eq("owner_id", session_data.value.data.session.user.id)
      .limit(1)
      .maybeSingle();
    ...
    companies_im_partof.value = companies.filter(
      (company) => company.company_id != my_company.value?.id
    );

    companies_im_partof.value.forEach(async (item) => {
      const { data: companyData } = await supabase
        .from("company_summary_view")
        .select("*")
